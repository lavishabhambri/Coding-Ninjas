...
int main() {
    ...
    int prev;
    cin>>prev;
    bool isDec = true;              //assuming initially to be decreasing.
    bool a = true;
    for(int i=1; i<n; i++){
        int curr;
        cin>>curr;
        if (curr == prev){   //as the function should be either strictly increasing or strictly decreasing.
            a = false;
            break;
        }
        
        else if(curr>prev){        //increasing
            if(isDec == true){
                isDec = false;
            ...
            if(isDec == false){   //means we are inc, then dec..so invalid
                //cout<<"false";
                a = false;
                break;
            }
            else{    //means we are inc, then dec..so invalid
                isDec = true;
